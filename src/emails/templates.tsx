...
const DEFAULT_SITE = process.env.NEXT_PUBLIC_SITE_URL ?? "https://online-store-seven-ashen.vercel.app";

const sansStack = "'Helvetica Neue', Helvetica, Arial, sans-serif";
const serifStack = "Georgia, 'Times New Roman', serif";

/* ── Shell reutilizable ── */
function EmailLayout({ preview, baseUrl, children }: { preview: string; baseUrl: string; children: React.ReactNode }) {
  return (
    <Html lang="es">
      <Head />
      ...
      <Body style={{ backgroundColor: C.cremaDark, margin: 0, padding: "28px 0", fontFamily: sansStack }}>
        <Container style={{ maxWidth: 560, margin: "0 auto", backgroundColor: C.crema, border: `2px solid ${C.moradoDark}` }}>
          ...
          <Section style={{ padding: "30px 0 16px", textAlign: "center" as const }}>
            <Img src={`${baseUrl}/logo-rdb.png`} alt="La Reina de Bastos" width="118" style={{ margin: "0 auto" }} />
...
export function BookingReceivedEmail(p: {
  baseUrl?: string; clientName?: string; service?: string; date?: string; format?: string; email?: string; phone?: string; notes?: string;
}) {
  const { baseUrl = DEFAULT_SITE, clientName = "María González", service = "Lectura de Tarot", date = "Jue 19 jun · 18:00 hs", format = "Zoom en vivo", email = "maria@ejemplo.com", phone = "+54 9 11 2345 6789", notes = "Prefiero la tarde. ¡Gracias!" } = p;
  return (
    <EmailLayout preview={`Nueva reserva de ${clientName}`} baseUrl={baseUrl}>
      ...
      <CTA href={`${baseUrl}/admin/reservas`}>Ver en el panel</CTA>
    </EmailLayout>
  );
}

export function BookingConfirmationEmail(p: { baseUrl?: string; clientName?: string; service?: string; date?: string }) {
  const { baseUrl = DEFAULT_SITE, clientName = "María", service = "Lectura de Tarot", date = "Jueves 19 de junio, 18:00 hs" } = p;
  return (
    <EmailLayout preview="Recibimos tu reserva ✦" baseUrl={baseUrl}>
      ...
      <CTA href={`${baseUrl}/mi-cuenta`} color={C.morado}>Ver mi cuenta</CTA>
    </EmailLayout>
  );
}

export function WelcomeCircleEmail(p: { baseUrl?: string; name?: string }) {
  const { baseUrl = DEFAULT_SITE, name = "María" } = p;
  return (
    <EmailLayout preview="Bienvenida al Círculo ✦" baseUrl={baseUrl}>
      ...
      <CTA href={`${baseUrl}/circulo`}>Entrar al Círculo</CTA>
...
export function OrderConfirmationEmail(p: { baseUrl?: string; name?: string; items?: { name: string; price: number }[]; total?: number }) {
  const { baseUrl = DEFAULT_SITE, name = "María", items = [{ name: "Tarot desde Cero", price: 89 }, { name: "Kit de Inicio Ritual", price: 45 }], total = 134 } = p;
  return (
    <EmailLayout preview="Tu compra está confirmada" baseUrl={baseUrl}>
...
export function NewPostEmail(p: { baseUrl?: string; title?: string; excerpt?: string; slug?: string }) {
  const { baseUrl = DEFAULT_SITE, title = "Ritual de Luna Nueva del mes", excerpt = "Un ritual completo para plantar intenciones en este ciclo.", slug = "ritual-de-luna-nueva-del-mes" } = p;
  return (
    <EmailLayout preview={`Nuevo en el Círculo: ${title}`} baseUrl={baseUrl}>
      ...
      <H>{title}</H>
      <P>{excerpt}</P>
      <CTA href={`${baseUrl}/circulo/${slug}`}>Leer ahora</CTA>
    </EmailLayout>
  );
}
...
export type EmailEntry = {
  ...
  accent: string;
  render: (baseUrl: string) => React.ReactElement;
};
...
export const emailRegistry: EmailEntry[] = [
  {
    ...
    to: "La nueva socia",
    accent: C.morado,
    render: (b) => <WelcomeCircleEmail baseUrl={b} />,
  },
  {
    key: "new-post",
    ...
    to: "Todas las socias",
    accent: C.dorado,
    render: (b) => <NewPostEmail baseUrl={b} />,
  },
  {
    ...
    to: "La clienta",
    accent: C.verde,
    render: (b) => <BookingConfirmationEmail baseUrl={b} />,
  },
  {
    ...
    to: "Belén (admin)",
    accent: C.rosa,
    render: (b) => <BookingReceivedEmail baseUrl={b} />,
  },
  {
    ...
    to: "Quien compró",
    accent: C.dorado,
    render: (b) => <OrderConfirmationEmail baseUrl={b} />,
  },
];

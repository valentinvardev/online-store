      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="flex items-center justify-between mt-auto">
          <div className="flex items-baseline gap-2">
            ...
            {priceOld && <span className="font-sans text-xs text-tierra/35 line-through">${priceOld}</span>}
          </div>
...
export default function EditarProductoPage() {
  ...
  const { data: producto, isLoading } = api.admin.productos.getById.useQuery(id, { enabled: !!id });
  ...
  useEffect(() => {
    if (producto && !initialized) {
      setForm({
        name: producto.name,
        description: producto.description,
        type: producto.type as "FISICO" | "DIGITAL" | "PERSONALIZADO",
        price: String(producto.price),
        priceOld: producto.priceOld ? String(producto.priceOld) : "",
        badge: producto.badge ?? "",
        fileUrl: producto.fileUrl ?? "",
        stock: producto.stock != null ? String(producto.stock) : "",
        videoUrl: producto.videoUrl ?? "",
      });
      if (producto.images?.length) {
        gallery.setInitialImages(producto.images.map((url: string) => ({ url, uploading: false })));
      }
      setInitialized(true);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [producto, initialized]);

  const vimeoEmbed = parseVimeo(form.videoUrl);

  const update = api.admin.productos.update.useMutation({
    onSuccess: () => { toast("Producto actualizado"); router.push("/admin/productos"); },
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-24 gap-3 text-tierra/40">
        ...
        <span className="font-sans text-sm tracking-wide">Cargando producto...</span>
      </div>
    );
  }
  ...
  if (!producto) {
    return (
      <div className="py-24 text-center">
        <p className="font-sans text-tierra/40 text-sm">Producto no encontrado</p>
        <Link href="/admin/productos" className="font-sans text-morado text-sm hover:underline mt-4 block">
          ← Volver a productos
        </Link>
      </div>
    );
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        <p className="font-sans text-[0.58rem] text-tierra/30 tracking-[0.35em] uppercase mb-1">Editando producto</p>
        <h1 className="font-display text-5xl text-morado-dark tracking-wide uppercase leading-none">
          {producto.name}
        </h1>
        <p className="font-sans text-tierra/45 mt-2 tracking-wide text-sm">Modificá los datos del producto</p>
      </div>
                  ...
                  <div className="grid grid-cols-3 gap-3 mt-1">
                    {(["FISICO", "DIGITAL", "PERSONALIZADO"] as const).map((t) => (
                      <button key={t} type="button" onClick={() => set("type", t)}
                        className={`py-3 px-3 border-2 text-left transition-colors ${form.type === t ? "bg-morado-dark text-crema border-morado-dark" : "bg-white text-tierra/50 border-morado/15 hover:border-morado/40"}`}>
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-3">
              <button type="submit" disabled={saving || gallery.isUploading}
                ...
                {saving ? "Guardando..." : gallery.isUploading ? "Subiendo fotos..." : "Guardar cambios"}
              </button>
